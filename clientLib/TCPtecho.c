...
int	nfds, ccount, hcount;
{
	...
	for (i=0; i<BUFSIZE; ++i)	/* echo data	*/
		buf[i] = 'D';
	memcpy((char *)&rcfds, (char *)pafds, sizeof(rcfds));
	memcpy((char *)&wcfds, (char *)pafds, sizeof(wcfds));
	for (fd=0; fd<nfds; ++fd)
		rc[fd] = wc[fd] = ccount;
	...
	while (hcount) {
		memcpy((char *)&rfds,(char *)&rcfds, sizeof(rfds));
		memcpy((char *)&wfds, (char *)&wcfds, sizeof(wfds));
